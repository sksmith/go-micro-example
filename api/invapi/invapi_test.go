...
func TestCreateProductionEvent(t *testing.T) {
	...
	req, err := http.NewRequest("PUT", ts.URL+fmt.Sprintf("/inventory/v1/%s/productionEvent", tpe.Sku),
...
func TestCreateReservation(t *testing.T) {
	...
	req, err := http.NewRequest("PUT", ts.URL+fmt.Sprintf("/inventory/v1/%s/reservation", tr.Sku),
		bytes.NewReader(data))
	if err != nil {
		t.Fatal(err)
	}
